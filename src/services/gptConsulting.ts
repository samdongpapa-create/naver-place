...
export type UnifiedPaidImprovements = {
  ...
  photoChecklist: string[]; // 체크리스트
  competitorKeywordInsights: string; // 경쟁사 키워드 요약(문장/불릿)
  priceGuidance?: string; // 업종별 정책/팁
};

export type GuaranteedConsultingResult = {
  improvements: UnifiedPaidImprovements;
  recommendedKeywords: string[]; // 10개 권장
  unifiedText: string; // 한 방 복붙 텍스트
  predicted: {
    totalScore: number;
    totalGrade: string;
    scores: any;
  };
  attempts: number; // GPT 재시도 횟수
};
...
function extractRegionHint(address: string): string {
  ...
  if (!a) return "";
  return a.split(" ").slice(0, 2).join(" "); // "서울 종로구" 정도
}
...
function ensureCount(arr: string[], n: number) {
  const u = uniq(arr);
  while (u.length < n) u.push(u[u.length - 1] ? `${u[u.length - 1]} 추천` : "지역 맛집");
  return u.slice(0, n);
}
...
function buildUnifiedText(name: string, out: UnifiedPaidImprovements, recommendedKeywords: string[], predictedScore: number, predictedGrade: string) {
  const lines: string[] = [];
...
export async function generatePaidConsultingGuaranteed(args: {
  ...
  competitorTopKeywords?: string[]; // ✅ 추가: 경쟁사 상위 키워드(빈도 TOP)
  ...
  let feedback = "";
  let bestSim = { totalScore: args.scoredNow.totalScore, totalGrade: args.scoredNow.totalGrade, scores: args.scoredNow.scores };
...
- photoChecklist: 어떤 사진을 어떤 순서로 올릴지 체크리스트(대표/내부/시술전후/디자이너/가격표/주차)
- competitorKeywordInsights: 경쟁사 키워드에서 뽑은 인사이트(왜 쓰는지, 우리 매장에 어떻게 녹일지) 5~8줄
...
`.trim();
    ...
    const description = clampText(safeStr(imp.description), constraints.descriptionMin, constraints.descriptionMax);
    const directions = clampText(safeStr(imp.directions), constraints.directionsMin, constraints.directionsMax);
    ...
    const reviewRequestScripts = {
      ...
      friendly: safeStr(imp?.reviewRequestScripts?.friendly) || "오늘 스타일 마음에 드셨다면 사진 한 장과 함께 리뷰 남겨주시면 큰 힘이 돼요!",
      polite: safeStr(imp?.reviewRequestScripts?.polite) || "방문 감사드립니다. 이용 후기를 리뷰로 남겨주시면 서비스 개선에 큰 도움이 됩니다."
    };

    const ownerReplyTemplates = {
      satisfied: safeStr(imp?.ownerReplyTemplates?.satisfied) || "소중한 리뷰 감사합니다! 다음 방문에도 더 만족드릴 수 있도록 준비하겠습니다 😊",
      photoEncourage: safeStr(imp?.ownerReplyTemplates?.photoEncourage) || "리뷰 감사합니다! 다음엔 시술 전/후 사진도 함께 남겨주시면 다른 고객분들께 큰 도움이 됩니다 😊",
      repeatCustomer: safeStr(imp?.ownerReplyTemplates?.repeatCustomer) || "재방문 감사합니다! 다음 예약 시 더 편하게 안내드릴게요. 늘 최선을 다하겠습니다.",
      complaint: safeStr(imp?.ownerReplyTemplates?.complaint) || "불편을 드려 죄송합니다. 말씀 주신 부분을 확인 후 개선하겠습니다. 가능하시면 매장으로 연락 주시면 빠르게 도와드릴게요.",
      noShowOrDelay: safeStr(imp?.ownerReplyTemplates?.noShowOrDelay) || "방문 일정 변경이 필요하셨다면 미리 연락 주시면 더 원활히 안내드릴 수 있습니다. 다음 예약 때 편하게 조율 도와드릴게요."
    };
    ...
    const mustLandmarksHit = nearby.filter(x => description.includes(x) || directions.includes(x)).length;
    if (mustLandmarksHit < constraints.mustIncludeLandmarks) lacks.push(`랜드마크 ${constraints.mustIncludeLandmarks}개 미포함(현재 ${mustLandmarksHit}개)`);
    for (const w of constraints.mustInclude) {
      if (!description.includes(w) && !directions.includes(w)) lacks.push(`필수 요소 "${w}" 미포함`);
    }

    feedback =
      `목표점수 ${target} 미달(예상 ${sim.totalScore}점). 아래를 반드시 보완:\n` +
      (lacks.length ? `- ${lacks.join("\n- ")}\n` : "") +
      `점수 상세: ${JSON.stringify(sim.scores)}`;
  }

  // ✅ 3회 내 실패 시 “최고 점수”로 반환(그래도 포맷은 통일)
  const imp = lastOut?.improvements || {};
  const regionHint = extractRegionHint(args.placeData.address || "");
  const nearby = buildNearbyLandmarks(industry, args.placeData.address || "", args.placeData.name || "");
  const constraints = buildConstraints(industry);

  const improvements: UnifiedPaidImprovements = {
    description: clampText(safeStr(imp.description), constraints.descriptionMin, constraints.descriptionMax),
    directions: clampText(safeStr(imp.directions), constraints.directionsMin, constraints.directionsMax),
    ...
    competitorKeywordInsights:
      safeStr(imp.competitorKeywordInsights) ||
      `경쟁사는 '지역+서비스' 조합 키워드를 반복 사용합니다.\n- ${regionHint} + 서비스(예: 염색/펌/컷)\n- 랜드마크(예: ${nearby.join(", ")})로 생활권 검색 흡수\n- 예약/상담/디자이너 포인트로 전환 강화`,
    priceGuidance: safeStr(imp.priceGuidance) || constraints.pricePolicy
  };

  const recommendedKeywords = ensureCount(Array.isArray(lastOut?.recommendedKeywords) ? lastOut.recommendedKeywords : [], 10);
  const unifiedText = buildUnifiedText(args.placeData.name, improvements, recommendedKeywords, bestSim.totalScore, bestSim.totalGrade);
